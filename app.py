        ...
        with st.chat_message("assistant"):
            ...
            with st.expander("Sources used", expanded=st.session_state.sources_expanded):
                ...
                if not chunks:
                    ...
                    for source_num, (doc_title, doc_chunks) in enumerate(
                        grouped_chunks.items(), start=1
                    ):
                        ...
                        if section_names:
                            ...
                            if len(section_names) == 1:
                                ...
                                st.write(
                                    f"**Sections:** {', '.join(section_names[:3])}"
                                )
                            ...
                            if pdf_path.exists():
                                ...
                                with open(pdf_path, "rb") as f:
                                    ...
                                    st.download_button(
                                        ...
                                        key=f"dl_{source_num}_{source_file}",
                                    )
...
with tab_dates:
    ...
    st.markdown("### Academic Year Overview")

    st.markdown("- Welcome and Development Week")
    st.markdown("- Semester 1 teaching block")
    st.markdown("- Winter assessment period")
    st.markdown("- Semester 2 teaching block")
    st.markdown("- Spring assessment period")import base64
from collections import defaultdict
from html import escape
from pathlib import Path
...
with tab_dates:
    ...
    st.markdown("- Winter assessment period")
    st.markdown("- Semester 2 teaching block")
    st.markdown("- Spring assessment period")    if not text:
        return ""
